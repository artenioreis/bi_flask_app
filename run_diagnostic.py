...
def run_diagnostics():
    ...
    if success and columns:
        ...
        for col in columns[:20]:  # Mostrar apenas as 20 primeiras
            print(f"      • {col['COLUMN_NAME']}: {col['DATA_TYPE']}")
        if len(columns) > 20:
            print(f"      ... e mais {len(columns) - 20} colunas")
    print()
    ...
    # 11. Verificar estrutura da tabela VENDE
    print("11. ESTRUTURA DA TABELA 'vende':")
    query_columns = """
    SELECT COLUMN_NAME, DATA_TYPE
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_NAME = 'vende'
    ORDER BY ORDINAL_POSITION
    """
    success, columns = db.execute_query(query_columns)
    if success and columns:
    ...
    # 12. Testar query simples de clientes
    print("12. TESTANDO QUERY SIMPLES DE CLIENTES:")
    query_test = """
    SELECT TOP 10
        cl.Codigo,
        cl.Razao_Social,
    ...
    """
    success, result = db.execute_query(query_test)
    if success and result:
        print(f"   Registros encontrados: {len(result)}")
        for row in result[:5]:
            print(f"      • {row['Codigo']} - {row['Razao_Social']} ({row['Nome_guerra']})")
    else:
        print("   ❌ Erro na query")
    print()
