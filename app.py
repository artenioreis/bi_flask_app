...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'varejao_bi_farma_2026_final_v8'
...
def dashboard():
    ...
    CASE WHEN cl.Bloqueado IN (0, '0') THEN 'Não' ELSE 'Sim' END,
    ...
    (SELECT ISNULL(SUM(Vlr_TotalNota), 0) FROM NFSCB WITH (NOLOCK)
     WHERE Cod_Cliente = cl.Codigo AND Status = 'F' 
     AND MONTH(Dat_Emissao) = 1 AND YEAR(Dat_Emissao) = 2026)
    ...
    if filtro == 'vendedor' and valor:
        ...
        realizado = float(execute_query(f"SELECT ISNULL(SUM(Vlr_TotalNota), 0) FROM NFSCB WITH (NOLOCK) WHERE Cod_Vendedor = {cod_v} AND Status = 'F' AND MONTH(Dat_Emissao) = 1 AND YEAR(Dat_Emissao) = 2026")[0][0] or 0)
    ...
    for r in res_db:
        c = list(r)
        if float(c[4] or 0) <= 0: c[6] = 0
        meta_clie = obj_excel.get(c[0], 0)
        venda_atual = float(c[10] or 0)
        ating_clie = (venda_atual / meta_clie * 100) if meta_clie > 0 else 0
        c.extend([meta_clie, ating_clie])
