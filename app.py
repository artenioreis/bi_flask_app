...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'varejao_bi_farma_2026_final_v6'
...
def dashboard():
    ...
    vendedores = execute_query("SELECT Codigo, Nome_guerra FROM vende WHERE Bloqueado = 0 OR Bloqueado = '0' ORDER BY Nome_guerra")
    ...
    for r in res_db:
        c = list(r)
        if float(c[4]) <= 0: c[6] = 0 # Força "Em Dia" se débito total for 0
        
        meta_clie = obj_excel.get(c[0], 0)
        # CORREÇÃO DEFINITIVA DA VARIÁVEL AQUI
        atingimento_clie = (float(c[10]) / meta_clie * 100) if meta_clie > 0 else 0
        c.extend([meta_clie, atingimento_clie])
