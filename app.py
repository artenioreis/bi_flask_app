...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'varejao_bi_farma_2026_final_v10'
...
def dashboard():
    ...
    if filtro == 'vendedor' and valor:
        ...
        titulo_vendas = "Vendas Gerais (Empresa)"
    ...
    # --- 2. LISTA DE CLIENTES E CÁLCULO DOS OBJETIVOS (EXCEL) ---
    # QUERY AJUSTADA COM ISNULL PARA EVITAR TYPEERROR
    query_clie = """
    SELECT cl.Codigo, cl.Razao_Social, 
    ISNULL(cl.Limite_Credito, 0) AS Limite, 
    ISNULL(cl.Total_Debito, 0) AS Debito, 
    ...
    (SELECT ISNULL(SUM(Vlr_TotalNota), 0) FROM NFSCB WITH (NOLOCK) WHERE Cod_Cliente = cl.Codigo AND Status = 'F' AND MONTH(Dat_Emissao) = 1 AND YEAR(Dat_Emissao) = 2026) AS Venda
    ...
    for r in res_db:
        # Unpacking seguro: usa (valor or 0) para prevenir NoneType
        cod = r[0]
        razao = r[1]
        lim = float(r[2] or 0)
        deb = float(r[3] or 0)
        ...
        meta_excel = obj_excel.get(cod, 0)
        total_meta_clie += meta_excel
        total_venda_clie += venda
        
        ating = (venda / meta_excel * 100) if meta_excel > 0 else 0
        # Formato para o template: [cód, razão, bloqueado, limite, debito, ?, atraso, ?, ?, ?, venda, meta, atingimento]
        clientes_finais.append([cod, razao, 'Não', lim, deb, 0, atraso, '', '', 0, venda, meta_excel, ating])

    # Cálculos da 2ª linha de indicadores (Base Clientes/Excel)
...
def analise_cliente(cliente_id):
    ...
    cliente = res[0]
    query_titulos = f"""SELECT Num_Documento, Par_Documento, Vlr_Documento, Vlr_Saldo, Dat_Emissao, Dat_Vencimento, 
                       DATEDIFF(DAY, DATEADD(DAY, ISNULL(Qtd_DiaExtVct, 0), Dat_Vencimento), GETDATE()) AS DiasAtraso 
                       FROM CTREC WITH (NOLOCK) WHERE Cod_Cliente = {cliente_id} AND Cod_Estabe = 0 AND Vlr_Saldo > 0 AND Status IN ('A', 'P') ORDER BY Dat_Vencimento ASC"""
